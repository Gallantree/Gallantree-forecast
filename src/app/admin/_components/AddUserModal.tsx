...
import { type CreateUserPayload, createUser } from "../_actions";
      ...
      {open && (
        ...
        >
          ...
          <div className="flex flex-col gap-6 px-6 py-5 text-sm">
            <section>
              <div className="mb-3 text-xs font-semibold text-zinc-500">Personal details</div>
            ...
            <section className="border-t border-zinc-200 pt-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="User type" required>
                  <select
                    value={userType}
                    onChange={(e) => setUserType(e.target.value as CreateUserPayload["userType"])}
                    className="w-full rounded-md border border-zinc-300 px-3 py-2"
                  >
                    {USER_TYPES.map((u) => (
            ...
            <section className="border-t border-zinc-200 pt-5">
              <div className="mb-3 text-xs font-semibold text-zinc-500">Organisation</div>
              <div className="grid grid-cols-2 gap-4">
              ...
              <div className="mt-4">
                <Field label="Status">
                  <select
                    value={status}
                    onChange={(e) => setStatus(e.target.value as CreateUserPayload["status"])}
                    className="w-full rounded-md border border-zinc-300 px-3 py-2"
                  >
                    {STATUSES.map((s) => (
...
function Field({
  ...
  return (
    <label className="flex flex-col gap-1">
      ...
      {hint ? <span className="text-[11px] text-zinc-500 -mt-0.5">{hint}</span> : null}
      {children}
    </label>
  );
}
